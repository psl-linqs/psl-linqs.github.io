...
In this changelog, you will find a list of the major changes in `2.2.0` as well as information on migrating from `2.1.0`.
...
while the `develop` branch stays up-to-date with our development work.
 ...
 - [Infrastructure](#infrastructure)
   ...
   - [API Reference Hosted on psl.linqs.org](#api-reference-hosted-on-psl.linqs.org)
...
have been moved to [psl.linqs.org/wiki](https://psl.linqs.org/wiki/).
...
Along with the Wiki, the API reference will now also be hosted at [psl.linqs.org/api](https://psl.linqs.org/api/).
...
## **PSL Interfaces**
The PSL 2.2.0 release comes with two new interfaces, and one deprecation.
...
```
The source for the interface is available in the [main PSL repository](https://github.com/linqs/psl/tree/2.2.0/psl-python).
...
[Functional predicates](https://psl.linqs.org/wiki/2.2.0/External-Functions.html) are now supported in the CLI.
...
Below you can see an example of the same model and data in PSL 2.1.0 (left) vs PSL 2.2.0 (right).
The blue portion of the graph is the actual memory being used.

![]({{ "/assets/images/2.2.0/psl-memory-improvements-2.2.0-opt.png" | relative_url }}){: .center-image }
...
We have added the [`FloatMatrix`](https://psl.linqs.org/api/2.2.0/org/linqs/psl/util/FloatMatrix.html) class to handle low-level matrix operations.
...
```
eriq@comp:~/code/psl-examples/simple-acquaintances/cli$ ./run.sh -D runtimestats.collect=true
...
eriq@comp:~/code/psl-examples/simple-acquaintances/cli$
```
...
With the release of PSL 2.2.0, we are adding a new weight learning method called Gaussian Process Prior (GPP), which is based on Bayesian optimization.
...
In contrast, likelihood-based methods (e.g. [Maximum Likelihood MPE](https://psl.linqs.org/api/2.2.0/org/linqs/psl/application/learning/weight/maxlikelihood/MaxLikelihoodMPE.html)) maximize the likelihood,
...
The class for GPP is [`org.linqs.psl.application.learning.weight.bayesian.GaussianProcessPrior`](https://psl.linqs.org/api/2.2.0/org/linqs/psl/application/learning/weight/bayesian/GaussianProcessPrior.html).
You can choose an evaulator by setting the `weightlearning.evaluator` configuration option to any [Evaluator](https://psl.linqs.org/api/2.2.0/org/linqs/psl/evaluation/statistics/Evaluator.html).
...
Domain: Any [Evaluator](https://psl.linqs.org/api/2.2.0/org/linqs/psl/evaluation/statistics/Evaluator.html)  
Default: [ContinuousEvaluator](https://psl.linqs.org/api/2.2.0/org/linqs/psl/evaluation/statistics/ContinuousEvaluator.html)  
