...
Bayesian Optimization for Weight Learning (BOWL) is a new weight learning approach for PSL which makes use of Gausian process priors in a Bayesian optimization setting to search for the best set of weights for rules. BOWL works by approximating a user defined metric function and evaluating the PSL model on N strategically chosen weights. It returns the set of weights that maximize this approximation. For further details about BOWL please see: https://linqs.soe.ucsc.edu/node/355
...
E.g.:
```
java -jar psl.jar --model test.psl --data test_learn.data --learn org.linqs.psl.application.learning.weight.bayesian.GaussianProcessPrior --eval ContinuousEvaluator
```

BOWL has three main hyperparameters that can be tuned for better results:
