...
This can be done by passing in multiple evaluators to the `--eval` argument:
```
...
Where possible, standard types of have been replaced by their shorter sibling
...
Bayesian Optimization for Weight Learning (BOWL) is a new weight learning approach for PSL which makes use of Gausian process priors in a Bayesian optimization setting to search for the best set of weights for the rules. As an input, BOWL takes in a user-defined metric to optimize and computes an approximate metric function by evaluating PSL model with N strategically chosen weights for the rules. In the end it returns weights that maximize this approximation. For further details about the please see: https://linqs.soe.ucsc.edu/node/355

To make use of this approach in the PSL open source package, the class name (org.linqs.psl.application.learning.weight.bayesian.GaussianProcessPrior) should be passed to the -learn argument and provided with a metric function to optimize using the --eval parameter.

E.g.:
```
java -jar psl.jar --model test.psl --data test_learn.data --learn org.linqs.psl.application.learning.weight.bayesian.GaussianProcessPrior --eval org.linqs.psl.evaluation.statistics.ContinuousEvaluator
```

BOWL has three main hyperparameters that can be modified for better results:
1. -D gpp.maxiterations=25
This sets the number of evaluations BOWL will conduct before choosing the best set of weights. 
NOTE: A number of 100 should be reasonable. Keep in mind, time taken to perform full learning in BOWL grows quadratically to the number of iterations. Therefore, if you choose a large number (such as 500), learning might take days to finish.

2. -D gppker.reldep=1.0
This is the relative dependence value given in GPP. 
The exploration space increases as the number of rules in the model increases. 
A smaller value would imply that weights are very distinct are also related and hence require lesser number of iterations. 
The value can be a positive floating point number.

3. -D gpp.explore=2.0
This parameter determines how the weights will be chosen for evaluation.
Lower value implies, the weights chosen for evaluation will be clustered around one region.
Higher value will lead to exploration of weights that are as distinct as possible.
