...
TODO(eriq): Table of Contents

TODO(eriq): Make content hierarchical?

TODO(eriq): Include migration information for each change.

TODO(eriq): Reference psl-examples

 0. [New Canary Naming System](#new-canary-naming-system)
 0. [PostgreSQL Database Backend](#postgresql-database-backend)
 0. [UniqueID Storage Types](#uniqueid-storage-types)
 0. [Removed InserterUtils](#removed-inserterutils)
 0. [External Function Support](#external-function-support)
 0. [Removed the Queries Class](#removed-the-queries-class)
 0. [Bulk Dataloading in Postgres](#bulk-dataloading-in-postgres)
 0. [Deferred Indexing](#deferred-indexing)
 0. [No More Predicate Deserialization](#no-more-predicate-deserialization)
 0. [Removed MySQL support](#removed-mysql-support)
 0. [Moved psl-evaluation](#moved-psl-evaluation)
 0. [Evaluation Infrastructure Changes](#evaluation-infrastructure-changes)
 0. [New Weight Learning Methods](#new-weight-learning-methods)
 0. [Configuration Reworked](#configuration-reworked)
 0. [Random in PSL](#random-in-psl)
 0. [Parallelization in PSL](#parallelization-in-psl)
 0. [Inference API Rework](#inference-api-rework)
 0. [Long Predicate Names Allowed](#long-predicate-names-allowed)
 0. [Getting the PSL Version](#getting-the-psl-version)
 0. [CLI Configuration Properties](#cli-configuration-properties)
 0. [Complete CLI Predicate Typing](#complete-cli-predicate-typing)

---

TODO(eriq): Reorder content (order of importance maybe?).
...
Unfortunately, we do not yet have any fancy infrastructure to check what canary versions are available, so the easiest way will be to just look at our [maven repository](https://linqs-data.soe.ucsc.edu/maven/repositories/psl-releases/org/linqs/psl-groovy/).
...
This means that you no longer will need the `psl-dataloading` dependency in you pom file.
...
In typical use cases, people will construct a `DataStore`, insert the data, and then get request database for weightlearning/inference/evaluation.
...
## Random in PSL
Random values can now be staticly fetched from the `org.linqs.psl.util.RandUtils` class.
This class is thread-safe.
Seeds can be set using the `RandUtils.seed` method or by setting the `random.seed` property.

## Parallelization in PSL
PSL now leverages milti-threading in grounding and term generation.
...
In previous versions of PSL, predicate names were limited to a certian length (which was decided by the database backend).
...
Previously, all predicte arguments in the CLI were typed as `UniqueStringID`.
