...
With the release of PSL 2.2.0, we are now adding a new weight learning algorithm called Gaussian Process Prior (GPP), which is based on bayesian optimization.

GPP is a search based method that works by approximating a user defined metric and evaluating the PSL model on a number of strategically chosen weights. Although GPP tends to work better with a smooth metric, it works for any chosen metric depending on the problem being solved. The major benefit of this is that the metric being optimized can be the same as that on which the results are being evaluated. GPP returns the set of weights that maximize this metric. For further details about GPP please see: [BOWL Bayesian Optimization for Weight Learning in Probabilistic Soft Logic](https://linqs.soe.ucsc.edu/node/355).

The class name [`GaussianProcessPrior`](https://psl.linqs.org/api/2.2.0/org/linqs/psl/application/learning/weight/bayesian/GaussianProcessPrior.html) should be used to use GPP in PSL.

For example, to use GPP in the CLI:
```
./run.sh --learn GaussianProcessPrior -D weightlearning.evaluator=ContinuousEvaluator
```

GPP has three main hyperparameters that can be tuned for better results:
* `-D weightlearning.evaluator=ContinuousEvaluator`    
This parameter specifies the user defined metric function that GPP uses to evaluate and optimize weights. The evaluator used depends on the task at hand. For example, to predict the level of trust between individuals ([trust prediction](https://github.com/linqs/psl-examples/tree/master/trust-prediction)) a `RankingEvaluator` may be used.

* `-D gpp.maxiterations=25`    
This sets the maximum number of times that BOWL will conduct evaluations before choosing the best set of weights. 
NOTE: A value of 100 should be reasonable. Keep in mind that the time taken to perform full learning in BOWL grows quadratically with the number of iterations. Therefore, if you choose a large number (such as 500), learning might take days to finish.
