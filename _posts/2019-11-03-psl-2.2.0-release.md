...
With the release of PSL 2.2.0, we are now adding support for a new weight learning algorithm called Gaussian Process Prior (GPP) which is based on bayesian optimization.
GPP works by approximating a user defined metric function and evaluating the PSL model on a number of strategically chosen weights. It returns the set of weights that maximize this approximation. For further details about GPP please see: [BOWL Bayesian Optimization for Weight Learning in Probabilistic Soft Logic](https://linqs.soe.ucsc.edu/node/355)

For example, to use GPP in the PSL CLI, the class name `GaussianProcessPrior` should be passed to the `--learn` argument and the metric function to optmize should be passed to the `weightlearning.evaluator` configuration option.
...
This parameter specifies the user defined metric function that GPP uses to evaluate and optimize weights. The evaluator used depends on the task at hand. For example, to classify documents in one of several categories ([citation categories](https://github.com/linqs/psl-examples/tree/master/citation-categories)) a `CategoricalEvaluator` may be used.
