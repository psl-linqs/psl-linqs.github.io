...
```
Each rule begins with a weight associated with it. These weights represent how important the rule is relative to other rules. PSL supports a number of algorithms to learn these weights. With the release of PSL 2.2.0, we are now adding support for another weight learning algorithm called GPP (Gaussian Process Prior) which is  based on bayesian optimization of a user-specified domain based performance metric. To use this you can directly pass in the name to the --learn option when invoking PSL. e.g:
```
java -jar psl.jar --model test.psl --data test_learn.data --learn bayesian.GaussianProcessPrior
```

For further information on GPP you can refer the [research paper](LINK_TO_PAPER_GOES_HERE).
